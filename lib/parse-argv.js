module.exports = function parse (args) {
  ...
  let list = listIndex >= 0 ? args[listIndex + 1] : undefined
  let label = labelIndex >= 0 ? args[labelIndex + 1] : undefined
  let effort = effortIndex >= 0 ? validateArg(args[effortIndex + 1]) : undefined
...
function validateArg (arg) {
  if (!arg) return false
  return ['yes', 'y'].includes(arg.toLowerCase())
}
